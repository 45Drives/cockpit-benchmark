  ...
  <div id="benchmarkForm" class="centered-column w-full gap-content flex flex-col items-stretch text-left">
    ...
    <div>
      ...
      <button id="downloadBenchmarkBtn" class="btn btn-primary mt-2 ml-2"
        :disabled="testInProgress || !testCompleted">Download
        Report</button>
      <div ref="benchProg" class="progress-container progress-description-left mt-4" :class="{ hidden: hiddenClass1 }">
        <div class="progress-description">
          <div class="flex justify-between mb-1">
            <span ref="benchStatus" class="text-secondary float-left" :class="{ hidden: hiddenClass1 }">{{
                benchmarkFeedback
            }}</span>
            <span ref="benchPercent" class="text-secondary float-right" :class="{ hidden: hiddenClass1 }">{{
                progPercent.toFixed(1)
            }}%</span>
          </div>
          <div class="bg-gray-200 rounded-full overflow-hidden" :class="{ hidden: hiddenClass1 }">
            <div class="h-3 bg-slate-500 rounded-full" :style="{ width: `${progPercent}%` }">
            </div>
          </div>
        </div>
      </div>
      <div ref="benchOut" class="benchmark-output mt-6 text-center float-left" :class="{ hidden: hiddenClass2 }">
        ...
        <div ref="chartOut" class=" float-right" :class="{ hidden: hiddenClass2 }">
...
const hiddenClass1 = ref(true);
const hiddenClass2 = ref(true);
...
async function launchTests() {
  hiddenClass2.value = true;
  hiddenClass1.value = false;
